...
def extract_ltl_data(data, mesh, varname,
    ...
    output = data.weighted(weight).sum(dim=dim_integration)  # time
    output['norm_weight'] = weight.sum(dim=dim_integration).compute()
    output.name = varname
...
def extract_oope_data(data, mesh, mask_dom=None):
    ...
    output['norm_weight'] = weight.sum(dim=['x', 'y']).compute()
